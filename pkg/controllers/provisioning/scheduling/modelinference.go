...
func (m *ModelInference) Inject(ctx context.Context, pod *v1.Pod) error {
	...
	kept, allowed := lo.FilterReject(flavors, func(flavor llmazcoreapi.Flavor, _ int) bool {
		return len(flavor.NodeSelector) > 0
	})
	if len(allowed) > 0 || len(kept) == 0 {
		return nil
	}
	...
	// so that relaxation replaces our flavor requirements according to the order of flavors in the model or pod's annotation,
